...
const thinScroll = {
  ...
  "&::-webkit-scrollbar-thumb":       { background: "rgba(0,47,89,0.18)", borderRadius: "2px" },
...
const SectionLabel = ({ children, sx = {} }) => (
  <Typography sx={{
    ...
    textTransform: "uppercase", color: INK_3, mb: 1.5, ...sx,
  }}>
    {children}
  </Typography>
);

const StatCard = ({ Icon, label, value, trend, trendPositive, loading, accent }) => (
  <Box sx={{
    ...card, p: 2.5,
    ...
    "&::before": accent ? {
      ...
      backgroundColor: NAVY, borderRadius: "12px 0 0 12px",
    } : {},
  }}>
    <Box display="flex" alignItems="center" justifyContent="space-between">
      <Box sx={{
        width: 32, height: 32, borderRadius: "8px", backgroundColor: NAVY_LIGHT,
    ...
    {loading
      ...
      : <Typography sx={{
          fontSize: "2.25rem", fontWeight: 700, lineHeight: 1, color: INK,
  ...
  return (
    <Box mb={1.5}>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={0.5}>
        <Typography sx={{ fontSize: "0.75rem", color: INK_2, fontWeight: 500 }}>{label}</Typography>
        {loading
          ? <Skeleton width={44} height={13} />
          : <Box display="flex" alignItems="baseline" gap={0.4}>
              <Typography sx={{ fontSize: "0.78rem", fontWeight: 700, color: INK, fontVariantNumeric: "tabular-nums" }}>
                {count}
              </Typography>
              <Typography sx={{ fontSize: "0.66rem", color: INK_3 }}>{p}%</Typography>
            </Box>
        }
      </Box>
      {loading
        ? <Skeleton height={6} sx={{ borderRadius: 4 }} />
        : <Box sx={{ height: 6, borderRadius: 4, backgroundColor: `${color}18`, overflow: "hidden" }}>
            <Box sx={{
              width: `${p}%`, height: "100%", backgroundColor: color, borderRadius: 4,
...
const Dot = ({ color }) => (
  <Box component="span" sx={{
    ...
    backgroundColor: color, flexShrink: 0, mt: "7px",
  }} />
);
      ...
      <ListItem alignItems="flex-start" disablePadding sx={{ py: 1.25, gap: 1.25, display: "flex" }}>
        ...
        <ListItemText disableTypography primary={
          <Box display="flex" justifyContent="space-between" alignItems="flex-start" gap={1}>
            <Box minWidth={0}>
              <Typography sx={{ fontSize: "0.8rem", fontWeight: 600, color: INK, lineHeight: 1.35 }} noWrap>
                {item.entity_name}
              </Typography>
              <Typography sx={{ fontSize: "0.7rem", color: INK_3, mt: 0.25 }}>
                {item.entity_type} {verb}{item.performed_by ? ` · ${item.performed_by}` : ""}
              </Typography>
            </Box>
            <Tooltip title={dayjs(item.action_time).format("MMM D, YYYY h:mm A")} placement="left">
              <Typography sx={{ fontSize: "0.68rem", color: INK_3, whiteSpace: "nowrap", cursor: "default", mt: "1px", flexShrink: 0 }}>
...
const Dashboard = () => {
  ...
  const fetchActivity = useCallback(async () => {
    try {
      const token = localStorage.getItem("token");
      const { data } = await axios.get("http://localhost:5000/api/dashboard/recent-activity", {
        headers: { Authorization: `Bearer ${token}` },
      });
      setActivity(data);
  ...
  const ageGroups = [
    ...
    { label: "Young Adult (18–30)", value: ageDist.age_18_30,   color: AGE_COLORS[1] },
    ...
    { label: "Elderly (60+)",       value: ageDist.age_60_plus, color: AGE_COLORS[4] },
  ];

  const sectorRows = [
    { Icon: AccessibleIcon,     label: "Person with Disability", key: "pwd_count",    color: "#7c3aed" },
    { Icon: ElderlyIcon,        label: "Senior Citizen",         key: "senior_count", color: "#0369a1" },
    { Icon: FamilyRestroomIcon, label: "Solo Parent",            key: "solop_count",  color: "#16a34a" },
  ];

  const statCards = [
    {
      Icon: PeopleAltIcon, label: "Total Residents",
      value: counts.total_residents, accent: true,
      trend: counts.residents_this_month > 0 ? `+${counts.residents_this_month} this month` : null,
      trendPositive: true,
    },
    {
    ...
    {
      Icon: ManageAccountsIcon, label: "Active Accounts",
      value: counts.active_users, accent: true,
      trend: "Currently active", trendPositive: false,
    },
    {
      Icon: AssignmentIcon, label: "Eligibility Forms",
      value: counts.active_forms, accent: true,
  ...
  return (
    <>
      ...
      <Box sx={{ position: "fixed", top: NAVBAR_H, left: 0, right: 0, bottom: FOOTER_H, ...thinScroll }}>
        <Box sx={{ maxWidth: 1400, mx: "auto", px: { xs: 2, sm: 3, md: 4 }, py: 3 }}>
          ...
          <Box display="flex" justifyContent="space-between" alignItems="flex-end" mb={3}>
            <Box>
              <Typography sx={{
                ...
                textTransform: "uppercase", color: NAVY, mb: 0.5,
              }}>
                Overview
              </Typography>
              <Typography sx={{ fontSize: "1.75rem", fontWeight: 700, color: INK, lineHeight: 1.15, letterSpacing: "-0.02em" }}>
                Dashboard
              </Typography>
          ...
          <Box sx={{ display: "flex", gap: 2, alignItems: "flex-start" }}>
            ...
            <Box sx={{ flex: "0 0 58%", minWidth: 0 }}>
              ...
              <Box sx={{ display: "flex", gap: 2, mb: 2 }}>
                ...
                <Box sx={{ ...card, p: 2.5, flex: 1, minWidth: 0 }}>
                  ...
                  {loadS
                    ...
                    : genders.length === 0
                    ? <Typography sx={{ fontSize: "0.8rem", color: INK_3 }}>No data.</Typography>
                    : genders.map((g) => {
                        const GIcon =
                          g.sex === "Male" ? MaleIcon :
                          g.sex === "Female" ? FemaleIcon : WcIcon;
                        return (
                          <Box key={g.sex} mb={1.5}>
                            <Box display="flex" justifyContent="space-between" alignItems="center" mb={0.5}>
                              <Box display="flex" alignItems="center" gap={0.6}>
                                <GIcon sx={{ fontSize: 14, color: GENDER_COLORS[g.sex] ?? INK_3 }} />
                            ...
                            <Box sx={{ height: 6, borderRadius: 4, backgroundColor: `${GENDER_COLORS[g.sex] ?? "#6b7280"}18`, overflow: "hidden" }}>
                              <Box sx={{
                                width: `${pct(g.count, totalG)}%`, height: "100%",
                ...
                <Box sx={{ ...card, p: 2.5, flex: 1, minWidth: 0 }}>
                  ...
                  {loadS
                    ? Array.from({ length: 3 }).map((_, i) => (
                        <Box key={i} display="flex" justifyContent="space-between" py={1.1}>
                          ...
                          <Box key={key}>
                            <Box display="flex" justifyContent="space-between" alignItems="center" py={1.1}>
                              <Box display="flex" alignItems="center" gap={0.75}>
                                ...
                                <Typography sx={{ fontSize: "0.76rem", color: INK_2, fontWeight: 500 }}>
                                  {label}
                                </Typography>
                              </Box>
                              <Box display="flex" alignItems="baseline" gap={0.5}>
                                <Typography sx={{ fontSize: "1rem", fontWeight: 700, color: INK, fontVariantNumeric: "tabular-nums" }}>
                                  {count}
                                </Typography>
                                <Typography sx={{ fontSize: "0.66rem", color: INK_3 }}>{p}%</Typography>
                              </Box>
                            </Box>
                            {i < sectorRows.length - 1 && <Divider sx={{ borderColor: BORDER }} />}
                          </Box>
                        );
                      })
                  }
                  ...
                  <Box display="flex" flexWrap="wrap" gap={0.6}>
                    {loadS
                      ? Array.from({ length: 4 }).map((_, i) => (
                          <Skeleton key={i} width={72} height={24} sx={{ borderRadius: "5px" }} />
                        ))
                      : civil.map((c) => (
                          <Box key={c.civil_status} sx={{
                            px: 1, py: 0.35,
                            border: `1px solid ${BORDER}`, borderRadius: "5px",
                            backgroundColor: SURFACE,
                            display: "flex", gap: 0.6, alignItems: "baseline",
                          }}>
                            <Typography sx={{ fontSize: "0.7rem", color: INK_2, fontWeight: 500 }}>
                              {c.civil_status}
                            </Typography>
                            <Typography sx={{ fontSize: "0.7rem", fontWeight: 700, color: INK, fontVariantNumeric: "tabular-nums" }}>
                              {c.count}
                            </Typography>
                          </Box>
                        ))
              ...
              <Box sx={{ ...card }}>
                <Box sx={{
                  ...
                  <Typography sx={{ fontSize: "0.82rem", fontWeight: 700, color: INK, letterSpacing: "-0.01em" }}>
                    Recently Added Records
                  </Typography>
                  <Typography sx={{ fontSize: "0.68rem", color: INK_3 }}>Last 10</Typography>
                </Box>
                ...
                {loadS ? (
                  ...
                  <Box px={2.5} py={4} textAlign="center">
                    <Typography sx={{ fontSize: "0.8rem", color: INK_3 }}>No records yet.</Typography>
                  </Box>
                ) : (
                  <Table size="small">
                    <TableHead>
                      <TableRow sx={{ backgroundColor: SURFACE }}>
                        {["Name", "Type", "Added"].map((h) => (
                          <TableCell key={h} sx={{
                            ...
                            color: INK_3, borderBottom: `1px solid ${BORDER}`,
                            py: 1,
                    ...
                    <TableBody>
                      {records.map((r, idx) => {
                        const ts = RECORD_TYPE_STYLES[r.type] ?? { color: INK_3, bg: SURFACE, Icon: PeopleOutlineIcon };
                        const TIcon = ts.Icon;
                        return (
                          <TableRow key={`${r.type}-${r.id ?? idx}`} sx={{
                            "&:last-child td": { border: 0 },
                            "&:hover td": { backgroundColor: SURFACE },
                            transition: "background 0.1s",
                            cursor: "default",
                          }}>
                            <TableCell sx={{ pl: 2.5, py: 1.1, borderColor: BORDER, maxWidth: 160 }}>
                              <Typography sx={{ fontSize: "0.78rem", fontWeight: 600, color: INK }} noWrap>
                                {r.name}
                              </Typography>
                            </TableCell>
                            <TableCell sx={{ borderColor: BORDER, py: 1.1 }}>
                              <Box sx={{
                                ...
                                <Typography sx={{ fontSize: "0.68rem", fontWeight: 600, color: ts.color }}>
                                  {r.type}
                                </Typography>
                              </Box>
                            </TableCell>
                            <TableCell sx={{ pr: 2.5, borderColor: BORDER, py: 1.1 }}>
                              <Tooltip title={dayjs(r.created_at).format("MMM D, YYYY h:mm A")} placement="left">
                                <Typography sx={{ fontSize: "0.72rem", color: INK_3, cursor: "default" }} noWrap>
            ...
            {/* ════════════════════════  RIGHT COLUMN  (≈42%)  ═══════════════════════ */}
            <Box sx={{ flex: "0 0 calc(42% - 8px)", minWidth: 0 }}>
              ...
              <Box sx={{ display: "flex", gap: 2, mb: 2 }}>
                ...
                <Box sx={{
                  ...
                  <Typography sx={{ fontSize: "0.75rem", color: INK_2, lineHeight: 1.75, textAlign: "justify" }}>
                    To develop a vibrant community led by competent, dynamic, and committed
                    leaders with family-oriented, caring, loving, healthy, secured, and
                    empowered people living harmoniously and sustainably managing the social
                    environment.
                  </Typography>
                </Box>
                ...
                <Box sx={{
                  ...
                  <Divider sx={{ borderColor: BORDER }} />
                  <Typography sx={{ fontSize: "0.75rem", color: INK_2, lineHeight: 1.75, textAlign: "justify" }}>
                    To create a community of sustainable growth through the provision of
                    effective and efficient services for local governance that will improve
                    the quality of life of the people in the Barangay.
                  </Typography>
                </Box>
              ...
              <Box sx={{ ...card, display: "flex", flexDirection: "column" }}>
                    ...
                    {[
                      ...
                      <Tooltip key={label} title={label} placement="bottom">
                        <Box sx={{
                          ...
                          backgroundColor: color, cursor: "default", mt: "2px",
                        }} />
                      </Tooltip>
                    ))}
                  </Box>
                ...
                {/* ↓ maxHeight keeps this from expanding the column infinitely */}
                <Box sx={{ maxHeight: 480, px: 2, ...thinScroll }}>
                  {loadA ? (
                    <Box py={1.5}>
                      {Array.from({ length: 10 }).map((_, i) => (
                        <Box key={i} display="flex" gap={1.25} py={1.25} alignItems="flex-start">
                          <Skeleton variant="circular" width={8} height={8} sx={{ mt: "7px", flexShrink: 0 }} />
                          <Box flex={1}>
                            <Skeleton height={12} sx={{ mb: 0.4 }} />
                            <Skeleton height={10} width="55%" />
                          </Box>
                          <Skeleton width={46} height={10} sx={{ flexShrink: 0 }} />
                        </Box>
                      ))}
                    </Box>
                  ) : activity.length === 0 ? (
                    <Box py={4} textAlign="center">
                      <Typography sx={{ fontSize: "0.8rem", color: INK_3 }}>No recent activity.</Typography>
                    </Box>
                  ) : (
                    <List disablePadding>
                      {activity.map((item, i) => (
